...
class BiConvLSTMCell(nn.Module):
    ...
    def __init__(
        ...
        self.padding = (
            filter_size - 1
        ) // 2  # in this way the output has the same size
        ...
        for i in range(self.num_directions):
            ...
            for j in steps:
                ...
                out_gate = torch.sigmoid(
                    out_gate
                )  # torch.sigmoid al posto di f.sigmoid
                # apply tanh non linearity
                cell_gate = torch.tanh(cell_gate)  # torch.tanh al posto di f.tanh
                # compute current cell and hidden state
                prev_single_cell = (remember_gate * prev_single_cell) + (
                    in_gate * cell_gate
                )
...
class BiConvLSTM(nn.Module):
    ...
    def __init__(
        ...
        )  # the first one has a different number of input channels
    ...
    def forward(self, input_, state=None):
        ...
        for idlayer in range(self.num_layers):  # loop for every layer
            ...
            current_input, layer_next_state = self.cell_list[idlayer](
                current_input, layer_state
            )
