...
from models import BiConvLSTM, layers
...
class FCDenseNet(nn.Module):
    def __init__(
        ...
        **lstm_parameters
        ...
        for i in range(len(down_blocks)):
            ...
            if self.use_sa:
                self.saBlocksDown.append(
                    layers.SqueezeAttentionBlock(cur_channels_count, cur_channels_count)
                )
        ...
        self.add_module(
            "bottleneck",
            layers.Bottleneck(cur_channels_count, growth_rate, bottleneck_layers),
        )
        ...
        if self.use_lstm:
            if lstm_parameters["lstm_parameters"]["bidirectional"] == True:
                ...
                num_directions = 1
            self.biConvLSTM = BiConvLSTM.BiConvLSTM(
                prev_block_channels,
                prev_block_channels // num_directions,
                lstm_parameters["lstm_parameters"]["lstm_kernel_size"],
                lstm_parameters["lstm_parameters"]["lstm_num_layers"],
                lstm_parameters["lstm_parameters"]["bidirectional"],
                True,
            )
        ...
        for i in range(len(up_blocks) - 1):
            self.transUpBlocks.append(
                layers.TransitionUp(prev_block_channels, prev_block_channels)
            )
            ...
            cur_channels_count += prev_block_channels
            if self.use_sa:
                self.saBlocksUp.append(
                    layers.SqueezeAttentionBlock(
    ...
    def forward(self, x):
        ...
        if self.use_lstm:
            out = out.view(
                batch_size, self.seq_size, out.size(1), out.size(2), out.size(3)
            )
            out = out.transpose(0, 1).contiguous()
            out, state = self.biConvLSTM(out, None)
            out = out.transpose(0, 1).contiguous()
            out = out.view(
                batch_size * self.seq_size, out.size(2), out.size(3), out.size(4)
            )
        ...
        for i in range(len(self.up_blocks)):
            ...
            if i == self.block_to_cancel:
                out[:, self.feature_to_cancel, :, :] = (
                    out[:, self.feature_to_cancel, :, :] * 0
                )
...
def FCDenseNet57(
    ...
    **lstm_parameters
    ...
    return FCDenseNet(
        ...
        seq_size=seq_size,
        use_lstm=use_lstm,
        block_to_cancel=block_to_cancel,
        feature_to_cancel=feature_to_cancel,
