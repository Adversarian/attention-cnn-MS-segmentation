    ...
    def __call__(self, pic):
        if isinstance(pic, np.ndarray):
            ...
            if m > 0:
                label = label / m
            label = (
                label.transpose(0, 1).transpose(0, 2).squeeze().contiguous().float()
            )  # .long() #label size [128, 128]
...
class MSDataset(data.Dataset):
    ...
    def __getitem__(self, index):
        imgs = torch.Tensor(
            self.seq_size, 3, self.input_dim, self.input_dim
        )  # 1x3x128x128
        targets = torch.LongTensor(
            self.seq_size, self.input_dim, self.input_dim
        )  # 1x128x128
        pil_imgs = []
        pil_targets = []
